"""
Improved Monza race track with obstacles in 3D using FURY.
Uses real TUMFTM database with variable track width and smarter obstacle placement.
"""
...
class MonzaTrack3DWithObstaclesImproved:
    def __init__(self,
        ...
        """
        Initialize Monza track with obstacles and optional variable width.
        ...
        num_obstacles : int
            Number of boulder obstacles to place
    ...
    def _add_obstacles(self):
        """Add cuboidal block obstacles with smarter placement considering variable width"""
        ...
        # Place obstacles avoiding start/finish area
        for _ in range(self.num_obstacles):
            idx = np.random.randint(50, len(self.track_centerline) - 50)
            base_point = self.track_centerline[idx]

            # Get track width at this location
            total_width, width_left, width_right = self._get_track_width(idx)

            # Place obstacle within narrower range for variable width tracks
            max_offset = min(width_left, width_right) * 0.4
            offset = np.random.uniform(-max_offset, max_offset)

            # Calculate perpendicular
            if idx < len(self.track_centerline) - 1:
                direction = self.track_centerline[idx + 1] - self.track_centerline[idx]
            ...
            # Random box dimensions (width, height, depth)
            base_size = np.random.uniform(0.8, min(2.0, total_width * 0.15))
            box_width = base_size * np.random.uniform(0.8, 1.2)
            box_height = base_size * np.random.uniform(0.5, 1.5)
            box_depth = base_size * np.random.uniform(0.8, 1.2)

            # Position obstacle
            obstacle_x = base_point[0] + perpendicular[0] * offset
            obstacle_z = base_point[1] + perpendicular[1] * offset
            obstacle_y = box_height / 2  # Center at half height (boxes are centered)

            obstacle_centers.append([obstacle_x, obstacle_y, obstacle_z])
            obstacle_scales.append([box_width, box_height, box_depth])

            # Varied colors (gray to brown concrete blocks)
            color_choice = np.random.choice(['gray', 'brown', 'tan'])
            if color_choice == 'gray':
                gray_val = np.random.uniform(0.4, 0.6)
                obstacle_colors.append([gray_val, gray_val, gray_val])
            elif color_choice == 'brown':
                obstacle_colors.append([0.55, 0.45, 0.35])
            else:  # tan
                obstacle_colors.append([0.65, 0.55, 0.45])

            # Store bounding box info for collision detection
            self.obstacle_positions.append({
                'position': np.array([obstacle_x, obstacle_y, obstacle_z]),
                'dimensions': np.array([box_width, box_height, box_depth]),
                'type': 'box'
            })
        ...
        if obstacle_centers:
            blocks = actor.box(
                centers=np.array(obstacle_centers),
                colors=np.array(obstacle_colors),
                scales=np.array(obstacle_scales)
            )
            self.scene.add(blocks)
    ...
    def get_obstacles(self):
        """Return obstacle positions for SLAM"""
        return self.obstacle_positions


def show_environment(num_obstacles=20, use_variable_width=True):
    """Display the track with obstacles"""
    track = MonzaTrack3DWithObstaclesImproved(
        num_obstacles=num_obstacles,
        use_variable_width=use_variable_width
    )
    ...
    title = "Monza Track 3D - Improved with Obstacles (Variable Width)" if use_variable_width else \
            "Monza Track 3D - Improved with Obstacles (Constant Width)"
    ...
    print(f"\nObstacles: {len(track.obstacle_positions)}")
    print(f"Track width: {'Variable (from TUMFTM data)' if use_variable_width else f'Constant ({track.default_track_width}m)'}")

    showm.start()


if __name__ == "__main__":
    show_environment(num_obstacles=20, use_variable_width=True)
